...
public:	
	...
	template<class T>
	void plotp(T *x, T *y, size_t Cnt, CvScalar color, char type = '*',bool is_need_lined = true);
...
template<class T>
void CPlot::plotp(T *X, T *Y, size_t Cnt, CvScalar color, char type, bool is_need_lined)
{
	//对数据进行存储
	T tempX, tempY;
	vector<CvPoint2D64f>data;
...
// 	//printf("data count:%d\n", this->dataset.size());
// 	
// 	this->DrawPoint(this->Figure); //每次都是重新绘制
}
...
void CPlot::DrawPoint (IplImage *image)
{
